...
export default function TrafficNav({title, c, sel, setSel}) {
    ...
    const cTag = c.map((item, key) => <TailButton key={key} caption={item} bcolor={sel === item ? 'orange':'blue'} handleClick= {() => handleClick(item)}/>)
  ...
  return (
    <div className='w-full flex justify-start items-center my-5'>
      <div className='w-1/5 flex justify-start items-center'>교통사고{title}</div>
